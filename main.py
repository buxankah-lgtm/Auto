...
class ParkingSystemGUI:
    def __init__(self, root):
        ...
        self.root.configure(bg="#1E2A44")  # Dark blue background
        ...
        if not self.cap_entrance.isOpened():
            logger.error(f"Error: Could not open entrance camera (ID: {cfg.CAMERA_ID_ENTRANCE})")
        ...
        if not self.cap_exit.isOpened():
            logger.warning(f"Warning: Could not open exit camera (ID: {cfg.CAMERA_ID_EXIT}). Exit detection disabled.")
            self.cap_exit = None
        ...
        logger.info(f"Starting system with entrance camera ID {cfg.CAMERA_ID_ENTRANCE}...")
        self.update_video()
    ...
    def set_costhour(self):
        try:
            ...
            logger.info(f"Cost per hour set to {self.costhour} RUB")
        except ValueError as e:
            messagebox.showerror("Ошибка", f"Введите корректное число: {e}")
            logger.error(f"Invalid costhour input: {e}")
    ...
    def update_video(self):
        ...
        if not ret or frame is None or not isinstance(frame, np.ndarray):
            logger.warning("Failed to capture entrance frame")
    ...
    def process_frame(self, frame, is_entrance=True):
        try:
            ...
            logger.debug(
                f"{'Entrance' if is_entrance else 'Exit'} - texts: {texts}, confidences: {confidences}, images_bboxs: {images_bboxs}")
            ...
            if number != "UNKNOWN" and confidence > cfg.CONFIDENCE_THRESHOLD and len(number) >= 4:
                if self.is_duplicate_plate(number, is_entrance):
                    ...
                    logger.info(f"Duplicate plate skipped: {number}")
                else:
                    ...
                    if final_plate and len(plate_frames[final_plate]) >= cfg.MIN_DETECTIONS_FOR_FINAL:
                        ...
                        if is_entrance:
                            ...
                            if self.costhour is not None:
                                ...
                                self.conn.commit()
                                logger.info(f"Inserted entrance: {final_plate}, ID: {self.cursor.fetchone()[0]}")
                        else:
                            self.cursor.execute(
