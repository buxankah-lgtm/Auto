...
class ParkingSystemGUI:
    def __init__(self, root):
        ...
        # Кастомный стиль для виджетов
        self.style = ttk.Style()
        self.style.theme_use('clam')  # Используем тему Clam для кастомизации
        self.style.configure("Custom.TLabel", background="#1E2A44", foreground="#00B7EB", font=("Helvetica", 12))
        self.style.configure("Custom.TButton", background="#2E4057", foreground="#FFFFFF", font=("Helvetica", 10, "bold"))
        self.style.map("Custom.TButton", background=[("active", "#3A5FCD")], foreground=[("active", "#FFFFFF")])
        self.style.configure("Custom.TEntry", fieldbackground="#2E4057", foreground="#FFFFFF", insertcolor="#FFFFFF")
        self.style.configure("Custom.TFrame", background="#1E2A44")
        self.style.configure("Log.TText", background="#2E4057", foreground="#FFFFFF", font=("Arial", 10))

        # Заголовок
        self.title_label = ttk.Label(root, text="AUTO SCANNER", style="Custom.TLabel")
        self.title_label.configure(font=("Helvetica", 16, "bold"))
        ...
        recog_frame = ttk.Frame(main_frame, style="Custom.TFrame")
        recog_frame.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
        main_frame.columnconfigure(0, weight=1)
        main_frame.rowconfigure(0, weight=1)

        ttk.Label(recog_frame, text="РАСПОЗНАНИЕ", style="Custom.TLabel").grid(row=0, column=0, pady=5)
        self.video_label = ttk.Label(recog_frame, style="Custom.TLabel")
        self.video_label.grid(row=1, column=0, pady=5, sticky="nsew")
        recog_frame.rowconfigure(1, weight=1)
        ttk.Button(recog_frame, text="Входы", style="Custom.TButton", command=lambda: logger.info("Entrance button clicked")).grid(row=2, column=0, pady=5)
        ttk.Button(recog_frame, text="Выходы", style="Custom.TButton", command=lambda: logger.info("Exit button clicked")).grid(row=3, column=0, pady=5)

        # Секция цены
        price_frame = ttk.Frame(main_frame, style="Custom.TFrame")
        price_frame.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
        main_frame.columnconfigure(1, weight=1)

        ttk.Label(price_frame, text="ЧЕНА:", style="Custom.TLabel").grid(row=0, column=0, pady=5)
        self.cost_label = ttk.Label(price_frame, text="Введите цену за час (в рублях):", style="Custom.TLabel")
        self.cost_label.grid(row=1, column=0, pady=5)
        self.cost_entry = ttk.Entry(price_frame, style="Custom.TEntry")
        self.cost_entry.grid(row=2, column=0, pady=5)
        self.cost_button = ttk.Button(price_frame, text="Установить цену", style="Custom.TButton", command=self.set_costhour)
        self.cost_button.grid(row=3, column=0, pady=5)

        # Блоки Вход и Выход
        status_frame = ttk.Frame(root, style="Custom.TFrame")
        status_frame.pack(pady=10, padx=20, fill="x")

        ttk.Label(status_frame, text="Вход:", style="Custom.TLabel").grid(row=0, column=0, padx=10, pady=5)
        self.entrance_plate_label = ttk.Label(status_frame, text="Нет данных", style="Custom.TLabel")
        self.entrance_plate_label.grid(row=1, column=0, padx=10, pady=5)

        ttk.Label(status_frame, text="Выход:", style="Custom.TLabel").grid(row=0, column=1, padx=10, pady=5)
        self.exit_plate_label = ttk.Label(status_frame, text="Нет данных", style="Custom.TLabel")
        self.exit_plate_label.grid(row=1, column=1, padx=10, pady=5)

        # Статус и лог
        self.status_label = ttk.Label(root, text="Ожидание распознавания...", style="Custom.TLabel")
        self.status_label.pack(pady=10)
        self.log_text = Text(root, height=5, width=80, style="Log.TText")
    ...
    def is_duplicate_plate(self, plate, is_entrance=True):
        ...
        if is_entrance:
            if (self.last_entrance_plate == plate and
                ...
                logger.debug(f"Дубликат на входе: {plate}, время с последней записи: {(current_time - self.last_entrance_time).total_seconds()} сек.")
                return True
            ...
            if (self.last_exit_plate == plate and
                ...
                logger.debug(f"Дубликат на выходе: {plate}, время с последней записи: {(current_time - self.last_exit_time).total_seconds()} сек.")
                return True
    ...
    def update_video(self):
        ...
        self.video_label.image = imgtk  # Сохраняем ссылку, чтобы избежать сборки мусора
        ...
        try:
            ...
            with open("car_numbers.txt", "a", encoding="utf-8") as f:
                ...
                if plate_frames and accepted:
                    ...
                    logger.debug(f"Проверка финального номера: final_plate={final_plate}, len={len(plate_frames.get(final_plate, []))}, min_detections={cfg.MIN_DETECTIONS_FOR_FINAL}")
